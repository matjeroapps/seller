...
func storefrontRoutes() []RouteSpec {
	return []RouteSpec{
		...
		},
		{
			...
			Path:        "/v1/storefront/carts",
			OperationID: "createStorefrontCart",
			Summary:     "Create a host-scoped Cart capability",
			Description: "Creates an active Cart bound to the resolved Store host and returns its bearer cookie.",
			Tags:        []string{"Storefront", "Cart"},
			Responses:   storefrontResponses("Public Cart", coreclient.CartResponse{}),
		},
		{
			Method:      http.MethodGet,
			Path:        "/v1/storefront/carts",
			OperationID: "getStorefrontCart",
			Summary:     "Read Cart for resolved Store host",
			Description: "Reads the active Cart identified by the HttpOnly cart bearer cookie.",
			Tags:        []string{"Storefront", "Cart"},
			Responses:   storefrontResponses("Public Cart", coreclient.CartResponse{}),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/storefront/carts/items",
			OperationID: "addStorefrontCartItem",
			Summary:     "Add a public SKU to Cart",
			Description: "Adds a public SKU and quantity to the active Cart.",
			Tags:        []string{"Storefront", "Cart"},
			RequestBody: struct {
		...
		},
		{
			...
			Path:        "/v1/storefront/carts/items/{itemID}",
			OperationID: "updateStorefrontCartItem",
			Summary:     "Update Cart item quantity",
			Description: "Updates the item quantity in the active Cart.",
			Tags:        []string{"Storefront", "Cart"},
			Parameters:  []ParameterSpec{PathStringParam("itemID", "Cart item identifier")},
			RequestBody: struct {
				Quantity int64 `json:"quantity"`
			}{},
			Responses: storefrontResponses("Public Cart", coreclient.CartResponse{}),
		},
		{
			...
			Path:        "/v1/storefront/carts/items/{itemID}",
			OperationID: "removeStorefrontCartItem",
			Summary:     "Remove a Cart item",
			Description: "Removes an item from the active Cart.",
			Tags:        []string{"Storefront", "Cart"},
			Parameters:  []ParameterSpec{PathStringParam("itemID", "Cart item identifier")},
			Responses:   storefrontResponses("Public Cart", coreclient.CartResponse{}),
		},
		{
			...
			Description: "Creates a Checkout Session for the active Cart.",
			Tags:        []string{"Storefront", "Checkout"},
			Responses:   storefrontResponses("Checkout Session", coreclient.CheckoutSessionResponse{}),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/storefront/checkout/sessions/{sessionID}/finalize",
			OperationID: "finalizeCheckoutSession",
			Summary:     "Finalize Checkout Session",
			Description: "Finalizes an eligible Checkout Session and creates an Order.",
			Tags:        []string{"Storefront", "Checkout"},
			Parameters:  []ParameterSpec{PathStringParam("sessionID", "Checkout Session identifier")},
			RequestBody: coreclient.FinalizeRequest{},
			Responses:   storefrontResponses("Finalized Order", storefrontapi.OrderResponse{}),
		},
		{
			Method:      http.MethodGet,
			Path:        "/v1/storefront/orders/{orderID}",
			OperationID: "getGuestOrder",
			Summary:     "Read a Guest Order",
			Description: "Reads a Guest Order authorized by HttpOnly capability cookie.",
			Tags:        []string{"Storefront", "Orders"},
		...
		{
			Method:      http.MethodPost,
			Path:        "/v1/storefront/orders/{orderID}/cancel",
			OperationID: "cancelGuestOrder",
			Summary:     "Cancel a pending Guest Order",
			Description: "Cancels a pending Guest Order authorized by HttpOnly capability cookie.",
			Tags:        []string{"Storefront", "Orders"},
			Parameters:  []ParameterSpec{PathStringParam("orderID", "Order identifier")},
			Responses:   storefrontResponses("Guest Order", storefrontapi.OrderResponse{}),
		},
...
func sellerRoutes() []RouteSpec {
	return []RouteSpec{
		...
		},
		{
			...
			Auth:        true,
			Parameters:  []ParameterSpec{PathStringParam("store_id", "Store identifier")},
			Responses:   AuthReadResponses("Store domain collection", sellerapi.StoreDomainCollectionResponse{}),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/seller/stores/{store_id}/domains",
			OperationID: "requestCustomDomain",
			Summary:     "Request a custom domain for a store",
			Description: "Initiates custom domain ownership verification by generating a DNS TXT challenge. Requires store ownership.",
			Tags:        []string{"Store Domains"},
			Auth:        true,
			Parameters:  []ParameterSpec{PathStringParam("store_id", "Store identifier")},
			RequestBody: sellerapi.RequestCustomDomainRequest{},
			Responses:   AuthCreatedResponses("Custom domain requested", sellerapi.StoreDomainResponse{}),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/seller/stores/{store_id}/domains/{domain_id}/verify",
			OperationID: "verifyCustomDomain",
			Summary:     "Verify custom domain DNS TXT record",
			Description: "Triggers DNS check for domain ownership. Transitions domain status to verified on success. Does not make the domain active.",
			Tags:        []string{"Store Domains"},
			Auth:        true,
			Parameters: []ParameterSpec{
			...
			Responses: AuthOKResponses("Custom domain verified", sellerapi.StoreDomainResponse{}),
		},
		{
			Method:      http.MethodPost,
			Path:        "/v1/seller/stores/{store_id}/domains/{domain_id}/activate",
			OperationID: "activateCustomDomain",
			Summary:     "Activate verified custom domain",
			Description: "Sets verified custom domain as the primary domain for the store. The platform domain becomes an active secondary domain.",
			Tags:        []string{"Store Domains"},
			Auth:        true,
			Parameters: []ParameterSpec{
				PathStringParam("store_id", "Store identifier"),
