...
/**
 * The structured product page sections of the Matjero default theme.
 *
 * Core serves these sections already locale-projected to the request locale:
 * `content` holds the display strings directly and never an `en`/`ar` envelope,
 * so no renderer localizes again. Every renderer is typed per section kind, and
 * every one of them degrades to rendering nothing when the content is missing
 * or malformed rather than crashing the page or leaking raw JSON. Section types
 * this theme does not know are skipped silently.
 *
 * `final_cta` does not duplicate any purchase logic: its button is an in-page
 * anchor that hands the customer to the product's existing purchase control,
 * which already owns the cart, buy-now and checkout behaviour.
 */
...
function DescriptionSection({ content }: { content: Record<string, unknown> }) {
...
function HighlightsSection({ content }: { content: Record<string, unknown> }) {
...
function SpecificationsSection({ content }: { content: Record<string, unknown> }) {
...
function FinalCtaSection({ content, context }: { content: Record<string, unknown>; context: ThemeContext }) {
  const title = str(content.title);
  const body = str(content.body);
  if (!title && !body) {
    return null;
  ...
  const ctaLabel =
    action === 'buy_now' ? (context.locale === 'ar' ? 'اشتري الآن' : 'Buy Now') : context.copy.cart.addToCart;

  return (
    <section className="product-page-section product-page-section--final_cta">
      {title ? <h2 className="product-page-section__title">{title}</h2> : null}
      {body ? <p className="product-page-section__text">{body}</p> : null}
      <a className="button button--primary product-page-section__cta" href="#purchase-control">
        {ctaLabel}
      </a>
    </section>
  );
...
function SectionRenderer({ section, context }: { section: SectionLike; context: ThemeContext }) {
  ...
  switch (section.type) {
    ...
    default:
      // An unknown section type is a content problem, never a page problem.
      return null;
  }
}
...
export function ProductSections({
  ...
  // The public projection omits `enabled` entirely (Core serves only enabled
  // sections), so an absent flag means visible; only an explicit false hides.
