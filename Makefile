...
docker-build:
	...
	docker build -f docker/web-app.Dockerfile --build-arg WORKSPACE=@commerce/seller-web -t matjero-seller-web:local .
	docker build -f docker/go-app.Dockerfile --build-arg APP_PATH=./apps/storefront-api -t matjero-storefront-api:local .
	docker build -f docker/web-app.Dockerfile --build-arg WORKSPACE=@commerce/storefront-web -t matjero-storefront-web:local .
